...
export const getCurrentUser = async (session) => {
  return await db.user.findUnique({
    where: { id: session.id },
    select: { id: true, roles: true, username: true, isBanned: true },
  })
}
 ...
 * @returns {boolean}
 */
 export const isAuthorizedToCrudUser = ({id}: {id: typeof context.currentUser.id}): boolean => {
  return context.currentUser.id === id;
}
