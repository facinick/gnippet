...
export const upvote = async ({ input }: MutationupvoteArgs) => {
  ...
  if(vote) {
    switch(vote.value) {
      ...
      updatedEntity = entityType === 'COMMENT'
      ?
      db.comment.update({ where: { id: commentId }, data: { score: { decrement: 1 }}})
      :
      db.snippet.update({ where: { id: snippetId }, data: { score: { decrement: 1 }}})
      ...
      upsertedVote = db.vote.update({ where: { id: vote.id }, data: { value: 1} })
      cudAction = 'UPDATED'
      // downvote has been converted into an upvote, increase score by 2
      updatedEntity = entityType === 'COMMENT'
      ?
      db.comment.update({ where: { id: commentId }, data: { score: { increment: 2 }}})
      :
      db.snippet.update({ where: { id: snippetId }, data: { score: { increment: 2 }}})
  ...
  // user is trying to [upvote for first time]
    ...
    upsertedVote = db.vote.create({ data: { value: 1, entityType, userId, snippetId, commentId} })
    cudAction = 'CREATED'
    // increase score by 1
    updatedEntity = entityType === 'COMMENT'
      ?
      db.comment.update({ where: { id: commentId }, data: { score: { increment: 1 }}})
      :
      db.snippet.update({ where: { id: snippetId }, data: { score: { increment: 1 }}})
  }

  const [responseVote, responseEntity] = await db.$transaction([upsertedVote, updatedEntity])
...
export const downvote = async ({ input }: MutationupvoteArgs) => {
  ...
  if(vote) {
    switch(vote.value) {
      ...
      updatedEntity = entityType === 'COMMENT'
      ?
      db.comment.update({ where: { id: commentId }, data: { score: { decrement: 2 }}})
      :
      db.snippet.update({ where: { id: snippetId }, data: { score: { decrement: 2 }}})
